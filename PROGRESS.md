...
- **Último `/check`**: 2026-07-21 (sesión C1) — verde salvo **los dos fallos ambientales de siempre del contenedor cloud, ajenos a C1**: el pool de workerd (`@cloudflare/vitest-pool-workers`) **segfaulta** sobre `tenants/demo/reset.test.ts` (fichero NO tocado; segfaulta incluso en aislamiento — el `pnpm check` completo cae por él, 40/42 tareas), y el test de rate-limit de la API parpadea bajo carga paralela (pre-existente, sesión 24). **Cada suite pasa en aislamiento**: API **86/86** (11 nuevos de `move`/`requote`/`preferredUnitId`/seasons), `@logic-camp/config` **39/39** (14 nuevos de geometría del planning), `@logic-camp/ui` **54/54** (27 nuevos del test de contraste AA), `@tenant/demo` seed **12/12**, core 51/51, notifications 7/7, payments 20/20, cli 17/17. Typecheck/lint/build de los 31 targets verdes
