...
- **Último `/check`**: 2026-07-21 (sesión C5) — 40/42 tareas, verde salvo **el mismo fallo ambiental de siempre del contenedor cloud, ajeno a C5**: el pool de workerd (`@cloudflare/vitest-pool-workers`) **segfaulta** sobre `tenants/demo/reset.test.ts` (fichero NO tocado). `@tenant/demo` en aislamiento: **seed.test.ts 12/12 verde**, el propio segfault ocurre DESPUÉS de que los 12 tests pasen. El resto de paquetes tocados esta sesión, verificados en aislamiento: `@tenant/demo` typecheck/lint limpios (script nuevo), `@logic-camp/site` typecheck/lint/build limpios (Landing.astro + OG image), `@logic-camp/web` typecheck/lint/build limpios (217 páginas) + verificación Playwright en vivo (0 imágenes rotas, 0 errores de consola en las galerías que aún degradan por C-BUG-5). No se tocó código de API/config/ui esta sesión.
