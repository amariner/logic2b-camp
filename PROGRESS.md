...
- **Último `/check`**: 2026-07-21 (sesión C4) — verde salvo **los dos fallos ambientales de siempre del contenedor cloud, ajenos a C4**: el pool de workerd (`@cloudflare/vitest-pool-workers`) **segfaulta** sobre `tenants/demo/reset.test.ts` (fichero NO tocado; segfaulta incluso en aislamiento — el `pnpm check` completo cae por él), y el test de rate-limit de la API parpadea bajo carga paralela (pre-existente, ver sesión 24). **Cada suite pasa en aislamiento**: API **75/75** (13 nuevos de C4), `@logic-camp/config` **25/25** (3 nuevos de `inhouse`), `@tenant/demo` seed **12/12** (+1 cobertura de check-in), `@logic-camp/ui` **27/27** (+1 `Command`). Typecheck/lint/build de los 17 paquetes verdes
