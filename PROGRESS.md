...
- **Último `/check`**: 2026-07-21 (sesión C7) — verde salvo **dos fallos ambientales del contenedor cloud, ajenos a C7**: el pool de workerd (`@cloudflare/vitest-pool-workers`) **segfaulta** sobre `tenants/demo/reset.test.ts` (fichero NO tocado; segfaulta incluso en aislamiento), y el test de rate-limit de usuarios de la API parpadea bajo carga paralela (pre-existente, ver sesión 24). **Cada suite pasa en aislamiento**: API **65/65** (3 nuevos de `/map`), `@logic-camp/config` **22/22** (18 nuevos de plano), `@tenant/demo` seed **11/11** (+1 cobertura de plano), `@logic-camp/ui` **26/26**. Typecheck/lint/build de todo el monorepo verdes
