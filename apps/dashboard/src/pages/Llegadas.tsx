...
function Lista({
  ...
  return (
    <section className="min-w-0 flex-1">
      ...
      {items.length === 0 && <p className="px-4 py-2 text-[13px] text-tinta-suave">{vacio}</p>}
      <ul className="divide-y divide-arena/40">
        {items.map((b) => {
          ...
          return (
            <li key={b.id}>
              <button
                ...
                className="grid w-full grid-cols-[auto_1fr_auto] items-center gap-x-3 gap-y-0.5 px-4 py-2 text-left text-[13px] hover:bg-arena-suave/50"
              >
                ...
                <span className="tnum col-start-1 text-tinta-suave">
                  {b.unitCode ?? t('dia.sinUnidad')}
                </span>
                <span className="tnum col-start-2 text-tinta-suave">
                ...
                <span
                  className={`tnum col-start-3 justify-self-end text-[12px] font-medium ${
                    pendiente > 0 ? 'text-mar' : 'text-pino'
                  }`}
                >
                  {pendiente > 0
  ...
  return (
    <div className="flex h-full">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {cargando && <p className="p-6 text-[14px] text-tinta-suave">{t('dia.cargando')}</p>}
        {error && <p className="p-6 text-[14px] font-medium text-mar">{t('dia.error')}</p>}

        {!cargando && !error && (
          <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pb-4 lg:flex-row lg:divide-x lg:divide-arena/60">
            <Lista
