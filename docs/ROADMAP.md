...
Actualización 2026-08-17 (sesión 147): la Fase 6 conserva su cierre y completa
el contrato que ya prometía la bandeja: una solicitud presupuestada se convierte
mediante una reserva real precargada, atómica, idempotente y auditada; el cambio
de estado aislado falla cerrado.
...
| 4 · Web pública + niveles              | ✅ Hecho               | 2026-07-22 | Web completa en 6 idiomas, mostrador/funnel por nivel, SEO, media responsive y Workers Assets en el mismo deploy. Las seis fotos pendientes se descargaron en la sesión 40 y Lighthouse local quedó ≥95. La re-medición productiva es un gate de publicación, no trabajo funcional de la fase.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 7 · Notificaciones                     | ✅ Hecho local (v1)    | 2026-08-10 | ADR 0010/0043: `packages/notifications` puro (plantillas 6 idiomas testeadas, drivers Resend/noop), `notify.ts` con `notifications_log` (`sent/failed/disabled`), eventos de solicitud, reserva, recordatorio y sistema, on/off por evento y `waitUntil`. R12 añadió respuesta Zod, timeout 8 s, máximo dos intentos con la misma clave idempotente, correlación e intentos exactos sin body remoto. Queues/reenvío posterior siguen en BACKLOG; cuenta, dominio, rebote y entrega real requieren autorización y no están acreditados.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 8 · Pagos                              | ✅ Hecho (v1)          | 2026-08-10 | ADR 0011: `packages/payments` puro (interfaz `PaymentProvider`, adaptadores stripe/redsys/none), modos v1 `none`/`deposit`/`full`; solo web abre pasarela. Booking nace `pending` y confirma por webhook idempotente; cancelación/dashboard ejecutan refund. R12 cierra Stripe entrada/salida y Redsys callback/refund: versión, tipos, campos, firma, timeout, acuse funcional y ambigüedades quedan acotados. Sandbox, conciliación y versión SHA-256/SHA-512 del terminal siguen como gates; demo en `mode:'none'`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 10 · Modo demo                         | ✅ Hecho local         | 2026-08-04 | Reset nocturno con ancla móvil, conmutador, banner, acceso anónimo fail-closed, restablecimiento y gestor móvil están construidos. Web Analytics queda clasificado por credencial. `ui.logic2b.com` es el DS externo de referencia; no se duplica un Storybook en este repo sin necesidad demostrada.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
### Corte duradero R15 · 2026-08-11

La auditoría no visual de la sesión 133 confirma que no queda trabajo funcional
local oculto fuera de temas. R0–R8 y R10–R11 conservan evidencia; R12–R13 han
agotado su porción local y separan material de cliente, proveedor e
infraestructura; R14 continúa vetado. El detalle y los disparadores están en
`docs/AUDITORIA-R15-NO-VISUAL.md`.

El bundle compuesto cierra 13.539 enlaces internos en 417 HTML y los
presupuestos M6 siguen verdes. La revalidación global queda condicionada a que
el tenant visual concurrente `vinyes` vuelva a tener JSON válido; el frente no
visual no modifica ese trabajo ni presenta 54/63 tareas como un check completo.
...
| ------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **B0** | Fundación del DS Logic2B                                     | Materializar en `packages/ui` los tokens oklch, radios, fuentes, wordmark y primitivos shadcn compartidos. Modo claro/oscuro.                                                                                                                                                                     | `packages/ui` exporta primitivos tipados + tokens; un componente de muestra se pinta con la marca; `pnpm check` verde |
| **B1** | Dashboard sobre Logic2B UI                                   | Migrar `apps/dashboard` a los componentes/tokens del DS. Sidebar agrupada, wordmark, radius 10px. El **planning** conserva color por estado pero derivado de tokens semánticos.                                                                                                                   | El gestor se lee como "producto Logic2B"; densidad y velocidad intactas; usable a 1366px y móvil; foco AA             |
...
**Estado del Frente B** (actualizado 2026-08-12): **B0, B1, B2, B3, B4 y B5
HECHOS.** `packages/ui` es el DS real (tema, tokens, fuentes,
...
firma «powered by Logic2B». B5 cierra el remate transversal de captación y
soporte sin reabrir las decisiones visuales ya cerradas del resto del frente.

### Detalle por fase (checklists — se afinan en cada ADR)

**B0 · Fundación del DS Logic2B**

- [x] ADR de fase y contrato de marca/tokens fijados en ADR 0016/0017 y `BRAND.md`.
- [x] `packages/ui`: tokens `:root` + `.dark` y componentes reexportados.
- [x] Fuentes self-hosted subsetadas: Inter, Space Grotesk y Poppins del wordmark.
- [x] Wordmark de producto e isotipo auxiliar/firma disponibles como componentes/asset.
- [x] Primitivos compartidos con tests de render y accesibilidad.
- [x] Storybook decidido: `ui.logic2b.com` es la referencia externa; no se duplica sin un consumidor demostrado.

**B1 · Dashboard sobre Logic2B UI**

- [x] ADR 0017 fija el mapa de componentes, tokens y estados del planning.
- [x] Dashboard sobre el tema Logic2B, radio 10 px y fuentes reales.
- [x] Shell con wordmark y sidebar agrupada, incluida su variante móvil.
- [x] Pantallas actuales usan los primitivos compartidos donde reducen duplicación sin perder densidad.
- [x] Planning con tokens semánticos, escenario de 300 unidades y presupuesto de rendimiento.
- [x] AA, teclado, movimiento reducido y QA 1366/375 verificados en las entregas B1/C1/M.
...
- [ ] **[producción]** Re-medir Lighthouse ≥95 tras el próximo deploy autorizado.

**B3 · Landing de producto (venta al CEO)**

- [x] ADR 0016: raíz de `camp.logic2b.com`, Astro en `apps/site`, narrativa e idiomas.
- [x] Héroe, niveles/precios, web+gestor, planning, portfolio, FAQ y CTA.
- [x] Enlaces vivos a `/demo/`, `/admin/` y `/docs/`.
- [x] Formulario comercial con adaptador demo y contrato explícito de entrega.
- [x] SEO/OG/hreflang y wordmark Logic2B sobre el sistema visual vigente.

**B4 · Documentación de producto**

- [x] ADR 0025: audiencia, Astro dentro de `apps/site`, estructura e idiomas.
...
**B5 · Contacto directo transversal por WhatsApp — ✅ HECHO (ADR 0046, 2026-08-12)**
...
página y deja de competir con el pie. Andreu aceptó la decisión y el corte se
implementó sin mezclarlo con creación de temas.

- [x] **Destino único:** normalizar el número como `+34 626 432 316` y el enlace
      ...
      en la URL.
- [x] **Sitio Logic2B:** cubrir landing, precios, portfolio, documentación y
      legales de `apps/site`, sin competir con el banner de consentimiento, los
      diálogos ni el CTA principal de cada página.
- [x] **Webs de camping:** cubrir `/demo/` y todas las rutas `/demos/{slug}/` en
...
- [x] **Gestor:** estar disponible desde el login y dentro del shell del
...
- [x] **Un contrato, varias superficies:** número, URL base, etiquetas y reglas
...
- [x] **Accesibilidad y privacidad:** enlace real y usable con teclado, foco
...
- [x] **QA de colocación:** revisar visualmente 375 y 1366 px en home comercial,
...
- [x] **Decidir con Andreu antes de implementar:** aparición inmediata o tras
...
ADR 0046 fija aparición pública tras 280 px y retirada ante pie/overlays,
...
El bundle compuesto, las guardias y el QA navegable acreditan la decisión.
...
- ~~**B-iii — Relación con `ui.logic2b.com` / Storybook.**~~ → **resuelta
  2026-08-10**: `packages/ui` materializa en el repo los tokens/componentes que
  usa el producto; `ui.logic2b.com` sigue como referencia externa. No se crea un
  segundo catálogo sin una necesidad demostrada.
...
no se implementa de forma preventiva. La auditoría R9 comparó las tres anclas
con Montaña, Familiar y Parcela y dejó inicialmente la ola en espera. La
decisión explícita de Andreu del 2026-08-10 abre después D5-V y selecciona la
siguiente demo programada: **Càmping Riu Clar** sustituye el concepto Montaña
...
Vinyes y Els Tarongers. Aporta identidad de encinar, 150 unidades, fotografía
propia 10/10 y gestor reversible. Su evidencia funcional es una política demo
...
- 2026-08-12: **B5 aceptado e implementado: WhatsApp transversal con Logic2B.** Andreu pide
  ...
  como Logic2B para no simular un canal del camping. ADR 0046 fija posición,
  persistencia y copy; contrato, tres adaptadores, guardias y QA quedan cerrados.
- 2026-08-10: **Andreu abre D5-V y encarga los doce temas programados** — su
  ...
  cuarta demo, Camping La Duna añade la quinta y Camping El Delta cierra D5-V
  en 6/6, las tres tier 1. `serralta` abre D6-V; ninguna marca obtiene core,
  backend o servicio propio.
- 2026-08-10: **R9 dejó inicialmente D5-V en espera en 3/3** — la capacidad de la fábrica
  y los conceptos Montaña/Familiar/Parcela no son señal comercial. Montaña se
  reabre ante una objeción observada de costa/estacionalidad; Familiar ante una
  necesidad concreta de edades, servicios infantiles o picos de cambio;
  Parcela ante una reserva/venta bloqueada por medidas, conexiones o vehículo.
  Sin una de esas evidencias no se crean activos ni una cuarta marca. D6-V solo
  se evalúa después de completar D5-V con evidencia individual.
...
4. ~~Indexación de la demo~~ → resuelta: la landing y documentación de producto
   en la raíz son indexables; Cala Sereno bajo `/demo/` y las marcas ficticias
   bajo `/demos/*` conservan `noindex`.
...
- [x] 2ª tanda de fotos: seis activos descargados y optimizados en la sesión 40;
      no ampliar densidad sin una necesidad visual concreta.
...
- [x] Planning firma: seed denso con ancla móvil, colores/estados, bloqueos,
      gestos, modo oscuro y alternativa móvil verificados.
...
- [x] Reset nocturno por cron (ADR 0013 + ADR 0030): wipe+reseed atómico,
      ancla en el día real, banda de llegadas/salidas garantizada y cron
      `0 3 * * *` del tenant demo.
...
- Accesibilidad AA real: foco visible, contraste verificado, navegación por teclado del mostrador y `prefers-reduced-motion`.
- Probar siempre a 1366px y en móvil 375px antes de cerrar un cambio visible.

## Cómo retomar una sesión

1. `cd /Users/andreumariner/Desktop/proyectos/logic-camp`
2. Leer `PROGRESS.md`, `CLAUDE.md` y este ROADMAP.
3. Continuar el primer checkpoint incompleto de
   `docs/RUTA-DESARROLLO-CONTINUO.md`; este ROADMAP aporta estado de frentes, no
   sustituye el orden del objetivo duradero.

## Orden de ataque (~6h/semana)

- **Próxima revisión con Andreu** → no hay bloque funcional local no temático
  abierto tras B5; elegir de forma explícita un gate de cliente/producción o un
  alcance nuevo antes de escribir más código.
