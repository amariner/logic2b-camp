...
| 9 · Instancias + asistente | 🟨 Parcial | 2026-07-19 | ADR 0012 (diseño aceptado, implementación parcial): `packages/config` con `TenantConfig` (tasa+cancelación, con test) sustituye las constantes `TAX_POLICY`/`CANCEL_POLICY` duplicadas en `public.ts`/`admin.ts`; `tenants/_template/` completo y funcional (`write-seed.ts` genera SQL válido, typecheck limpio); `docs/ONBOARDING.md` primer borrador. Registro de extensiones (Fase 2, sin usar hasta hoy) queda diseñado pero sin conectar — no hay todavía ningún `custom/` real que lo necesite. `packages/cli` (`pnpm new:camping`) y crear un tenant de prueba real: bloqueados por credenciales/mandato sobre la cuenta de Cloudflare, no por falta de diseño |
