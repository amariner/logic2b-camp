...
| **Pagos**                         | Contratos `none/stripe/redsys`, importes en servidor, intents, firma webhook, idempotencia, duplicados, refund y fallo cerrado sin secret. Solo mocks/clave pública de test en suite; ningún sandbox acreditado.                                                                                                                 | Cliente titular del comercio; Stripe: `STRIPE_SECRET_KEY`+`STRIPE_WEBHOOK_SECRET`; Redsys: código/terminal+`REDSYS_MERCHANT_KEY`; política de cobro/reembolso. | Sandbox extremo a extremo: éxito, abandono, fallo, webhook repetido/importe incorrecto, reembolso, conciliación y disputa operativa. | `provider:none` evita pasarela; pausar checkout web sin alterar cobros manuales; reconciliar antes de volver.                  |
