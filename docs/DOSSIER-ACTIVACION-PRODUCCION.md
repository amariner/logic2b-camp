...
| **Pagos**                         | Contratos `none/stripe/redsys`, importes en servidor, intents, idempotencia D1, duplicados y refund. Stripe exige HMAC+máximo 300 s en entrada y, en salida, timeout 8 s, POST idempotente, Zod, un reintento seguro y error cerrado. Redsys verifica firma local; su refund aún no valida acuse funcional. Solo dobles/clave pública de test: ningún sandbox acreditado. | Cliente titular del comercio; Stripe: `STRIPE_SECRET_KEY`+`STRIPE_WEBHOOK_SECRET`; Redsys: código/terminal+`REDSYS_MERCHANT_KEY`; política de cobro/reembolso. Ver `RUNBOOK-PAGOS.md`. | Sandbox extremo a extremo: éxito, abandono, fallo, webhook repetido/importe incorrecto, ambigüedad sin duplicado, reembolso, conciliación y disputa operativa. | `provider:none` evita pasarela; pausar checkout web sin alterar cobros manuales; reconciliar antes de volver.                  |
