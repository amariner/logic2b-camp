...
| **Stripe**                           | parcial: config Zod en API; respuesta externa con casts                       | sí en booking/webhook D1; falta cabecera saliente en requests Stripe | no           | no                     | `payments.raw=null`; error remoto puede conservar texto | sí: `provider:none` y fallo cerrado sin secrets | parcial en dossier      | mocks locales; sandbox y comercio son gate                  |
