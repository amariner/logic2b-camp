...
| **Stripe**                           | parcial: webhook Zod; respuesta HTTP saliente aún casteada                    | sí en booking/webhook D1; falta cabecera saliente en POST Stripe   | no en salida | no                     | ingress sin payload; error saliente conserva texto   | sí: `provider:none` y fallo cerrado sin secrets | runbook + dossier       | firma local con antirreplay 300 s; sandbox sigue siendo gate |
| **Redsys**                           | parcial: config Zod; callback parseado y firmado                              | webhook/pago deduplicados en D1; pedido nuevo no declara reintento | no en refund | no                     | payload no se persiste; códigos cerrados             | sí: `provider:none`                             | parcial en dossier      | firma local/clave pública; FUC sandbox es gate               |
...
2. **Pagos:** revisar en una entrega separada la frontera HTTP de Stripe/Redsys,
   expiración de webhook, idempotencia saliente y respuesta externa. Ninguna
