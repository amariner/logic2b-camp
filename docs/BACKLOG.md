...
- ~~[R13/datos-locales] Probar migraciones, seed, owner y rollback sin tenant~~ →
  **hecho 2026-08-10 (sesión 128)**: `pnpm onboarding:rehearse <año>` opera en
  un único temporal sin variables de credenciales; aplica dos veces las ocho
  migraciones, compara dos seeds, valida inventario/auth/invariantes, exporta,
...
- ~~[8.x] Botón "reintentar el pago" en `/reserva` cuando `pago=cancelado`/queda `pending` mucho tiempo~~ → **hecho 2026-08-13 (sesión 143)**: el gestor público verifica código + email + estado y devuelve exactamente el intento persistido; nunca duplica el cobro ni llama al proveedor. La UI retoma redirect/form con copy en seis idiomas y conserva recepción como alternativa.
...
- ~~[C1] Al confirmar un movimiento con cambio de precio y `paidCents > total` nuevo, avisar inline en el propio diálogo de precio~~ → **hecho 2026-08-13 (sesión 144)**: `requote` devuelve lo cobrado y el diálogo calcula y explica el exceso antes de confirmar. Es solo informativo: mover no altera `paidCents` ni registra un reembolso; la acción explícita y auditada continúa en la ficha.
...
- [12] **SES.Hospedajes real**: modelo y validación local están implementados;
  el XML descargable se presenta expresamente como borrador para revisar y
  comunicar en la Sede. El adaptador provisional contra endpoint simulado se
  retiró: las credenciales reservadas no habilitan red ni acuse. Falta obtener
  desde el alta autenticada la documentación técnica vigente, fijar endpoint,
